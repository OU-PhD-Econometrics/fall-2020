...
These slides also heavily follow Chapter 2 of `r Citet(biblio,"train2009")`
...
# Probabilities
With the $\epsilon$'s unobserved, the probability of individual $i$ making choice $j$ is given by:
\begin{align*}
...
Can't tell whether men are happier than women, but can tell whether they more strongly prefer one option

Can only obtain differenced coefficient estimates on $X$'s

Can only obtain an estimate of a coefficient that is constant across choices if its corresponding variable varies by choice
...
Can rewrite the last line of \eqref{eq:intprob} as a $J-1$ dimensional integral over the differenced $\epsilon$'s:
\begin{align}
...
Consider the case when the choice set is $\{1,2\}$.  The Type 1 extreme value CDF for $\epsilon_2$ is:
\begin{align*}
...
.small[Adding more choices with i.i.d. Type I extreme value errors yields the .hi[multinomial logit]
